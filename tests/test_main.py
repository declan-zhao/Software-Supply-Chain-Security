...
class TestInclusion:
    ...
    def test_inclusion_file_not_found(self):
        ...
        with pytest.raises(
            FileNotFoundError, match="Artifact filepath invalid"
        ):
    ...
    def test_inclusion_success(
        ...
        mock_get_verification_proof.return_value = mock_log_entry["abc123"][
            "verification"
        ]["inclusionProof"]
...
class TestGetConsistencyProofData:
    ...
    def test_get_consistency_proof_data_invalid_first_size(self):
        ...
        with pytest.raises(
            ValueError, match="first_size must be a positive integer"
        ):
    ...
    def test_get_consistency_proof_data_invalid_last_size(self):
        ...
        with pytest.raises(
            ValueError, match="last_size must be a positive integer"
        ):
    ...
    def test_get_consistency_proof_data_invalid_tree_id(self):
        ...
        with pytest.raises(
            ValueError, match="tree_id must be a non-empty string"
        ):
...
class TestConsistency:
    ...
    def test_consistency_success(
        ...
        mock_get_consistency_proof.assert_called_once_with(
            10, 20, "123", False
        )
    ...
    def test_consistency_missing_tree_id(self, mock_get_latest_checkpoint):
        ...
        prev_checkpoint = {"treeSize": 10, "rootHash": "prev_root"}
        ...
        with patch("src.main.get_consistency_proof_data") as mock_get_proof:
            mock_get_proof.side_effect = ValueError(
                "tree_id must be a non-empty string"
            )
            with pytest.raises(ValueError):
                consistency(prev_checkpoint)
